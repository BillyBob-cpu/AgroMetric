...
DB_CONFIG = {
    ...
    'user':     os.getenv('DB_USER',     'agrometric'),
...
def create_tables():
    ...
    cur.execute("""
        CREATE TABLE IF NOT EXISTS parcelles (
            ...
            surface_ha  NUMERIC(6,2) NOT NULL
        );
...
def import_csv():
    ...
    with open(os.path.join(DATA_DIR, 'parcelles.csv'), encoding='utf-8') as f:
        for row in csv.DictReader(f):
            cur.execute(
                "INSERT INTO parcelles(id,nom,localisation,surface_ha) "
                "VALUES(%s,%s,%s,%s) ON CONFLICT(id) DO NOTHING",
                (row['id'], row['nom'], row['localisation'], row['surface_ha'])
            )

    with open(os.path.join(DATA_DIR, 'cultures.csv'), encoding='utf-8') as f:
        for row in csv.DictReader(f):
            cur.execute(
                "INSERT INTO cultures(id,type,date_semis,parcelle_id) "
                "VALUES(%s,%s,%s,%s) ON CONFLICT(id) DO NOTHING",
                (row['id'], row['type'], row['date_semis'], row['parcelle_id'])
            )

    with open(os.path.join(DATA_DIR, 'meteo.csv'), encoding='utf-8') as f:
        for row in csv.DictReader(f):
            cur.execute(
                "INSERT INTO meteo(date,temperature,humidite,pluie_mm) "
                "VALUES(%s,%s,%s,%s) ON CONFLICT(date) DO NOTHING",
                (row['date'], row['temperature'], row['humidite'], row['pluie_mm'])
            )

    with open(os.path.join(DATA_DIR, 'observations.csv'), encoding='utf-8') as f:
        for row in csv.DictReader(f):
            cur.execute(
                "INSERT INTO observations(date,etat,parcelle_id,commentaire) "
                "VALUES(%s,%s,%s,%s)",
                (row['date'], row['etat'], row['parcelle_id'], row.get('commentaire', ''))
            )

    with open(os.path.join(DATA_DIR, 'alertes.csv'), encoding='utf-8') as f:
        for row in csv.DictReader(f):
            cur.execute(
                "INSERT INTO alertes(date,type,parcelle_id,niveau) "
                "VALUES(%s,%s,%s,%s)",
                (row['date'], row['type'], row['parcelle_id'], row['niveau'])
            )
...
def get_all_parcelles():
    ...
    cur.execute("""
        SELECT
            p.id, p.nom, p.localisation, p.surface_ha,
            c.type AS culture_type,
            c.date_semis::text AS date_semis,
            (SELECT etat FROM observations
             WHERE parcelle_id = p.id
             ORDER BY date DESC LIMIT 1)        AS dernier_etat,
            (SELECT COUNT(*) FROM alertes
             WHERE parcelle_id = p.id
               AND niveau = 3)                  AS alertes_critiques
        FROM parcelles p
...
def get_meteo(limit=30):
    ...
    cur  = conn.cursor(cursor_factory=psycopg2.extras.RealDictCursor)
    cur.execute(
        "SELECT id, date::text, temperature, humidite, pluie_mm "
        "FROM meteo ORDER BY date DESC LIMIT %s",
        (limit,)
    )
    rows = cur.fetchall()
    cur.close(); conn.close()
...
def get_meteo_latest():
    ...
    cur.execute(
        "SELECT id, date::text, temperature, humidite, pluie_mm "
        "FROM meteo ORDER BY date DESC LIMIT 1"
    )
    row = cur.fetchone()
    cur.close(); conn.close()
    return dict(row) if row else None
...
def get_observations(parcelle_id=None):
    ...
    if parcelle_id:
        cur.execute(
            "SELECT id, date::text, etat, parcelle_id, commentaire "
            "FROM observations WHERE parcelle_id = %s ORDER BY date DESC",
            (parcelle_id,)
        )
    else:
        cur.execute(
...
def get_alertes(niveau=None):
    ...
    if niveau:
        cur.execute(
            "SELECT id, date::text, type, parcelle_id, niveau, source "
            "FROM alertes WHERE niveau = %s ORDER BY date DESC",
            (niveau,)
        )
    else:
        cur.execute(
            "SELECT id, date::text, type, parcelle_id, niveau, source "
            "FROM alertes ORDER BY date DESC"
        )
    rows = cur.fetchall()
    cur.close(); conn.close()
    return [dict(r) for r in rows]
...
def insert_alerte(data):
    ...
    cur  = conn.cursor()
    cur.execute(
        "INSERT INTO alertes(date,type,parcelle_id,niveau,source) VALUES(%s,%s,%s,%s,%s)",
...
# ══════════════════════════════════════════
#  REQUÊTES — AUTHENTIFICATION USERS
# ══════════════════════════════════════════

def create_user(nom, prenom, email, password, role='utilisateur'):
    """Crée un nouvel utilisateur. Retourne l'utilisateur créé, ou None si email déjà pris."""
    conn = get_conn()
    try:
        cur = conn.cursor()
...
def get_user_by_email(email):
    """Récupère un utilisateur par email (mot de passe hashé inclus pour vérification)."""
    conn = get_conn()
    try:
        cur = conn.cursor()
        cur.execute(
            "SELECT id, nom, prenom, email, password, role, created_at "
...
def get_all_users():
    """Retourne tous les utilisateurs inscrits (sans mot de passe)."""
    conn = get_conn()
    try:
        cur = conn.cursor()
        cur.execute(
        ...
        return [
            {'id': r[0], 'nom': r[1], 'prenom': r[2],
             'email': r[3], 'role': r[4], 'created_at': str(r[5])}
            for r in rows
        ]
    finally:
        cur.close(); conn.close()
